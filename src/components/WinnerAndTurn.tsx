...
const WinnerAndTurn: React.FC<WinnerAndTurnProps> = ({winner, turn}) => {
  ...
  return (
    <>
    {winner ? (
        <article className={`${winner === "Player 1" ? "bg-p1Bg" : "bg-p2Bg"} absolute text-white w-[250px] py-3 px-2 text-center rounded-2xl border-2 border-black shadow-boardShadow -bottom-[130px] left-0 right-0 mx-auto uppercase flex flex-col gap-y-3`}>
          <h2 className="font-bold text-lg">{winner}</h2>
          <span className="text-5xl font-medium">Wins</span>
          <button className="uppercase bg-secondaryBg px-5 py-[6px] rounded-full w-fit mx-auto font-semibold" onClick={askForNewGame}>Play Again</button>
        </article>
       ): (
    ...
    {newGame?.askedBy !== 0 && newGame?.askedBy !== currentPlayer?.id && (
      ...
      <article className="bg-white p-6 rounded-lg flex flex-col gap-y-6 items-center shadow-boardShadow border-4 border-black">
        <Logo />
        <h2 className="text-xl font-medium">Do you want to play one more game?</h2>
        <div className="w-full flex justify-between px-1">
          <button className="bg-red-500 text-white px-8 py-2 rounded-sm" onClick={() => handleNewGameAsk("No")}>No</button>
          <button className="bg-primaryBg text-white px-4 py-2 rounded-sm" onClick={() => handleNewGameAsk("Yes")}>Play One More</button>
        </div>
      </article>
      </div>
    )}
